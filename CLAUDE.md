...
All 44 checks are implemented across MDO (14), EXO (17), and Teams (13), plus `Test-METPrerequisites` for pre-flight dependency checks and `Disconnect-METSession` for session teardown. Console, JSON, and HTML report formats are all shipped. See `ROADMAP.md` for the full version history, `docs/gap-analysis-2026-08.md` for the research behind the 11 checks added in v0.6.0, `docs/gap-analysis-2026-08-teams-attack-surface.md` for the research behind v0.8.0's 5 new Teams checks (009-012, 014) and 5 enhancements to existing Teams checks (001, 003, 004, 006, 008), `docs/gap-analysis-2026-08-quarantine-policies.md` for v0.9.0's two confirmed quarantine-check false-positive bugs, and `docs/gap-analysis-2026-08-connect-session-security.md` for v0.10.0 - a confirmed cross-customer data leak in session reuse (no tenant/org verification before reusing a live EXO/Graph/Teams connection), fixed alongside certificate-file auth for non-Windows and a scoped-down, research-grounded device-code auth posture (Microsoft's own current guidance: "block wherever possible, allow only where necessary").
