...
- **An absent property never yields `Pass`** - a property the service did not return says nothing about the tenant's configuration. Absent on some objects but present on others -> `Warning`, naming the objects whose property was absent. Absent on every object -> `NotApplicable`, with `-ErrorMessage` recording that the property was not returned - typically a module or service version that does not expose it. A check may escalate to `Fail` where absence has a specific, documented meaning for that control; `MET-EXO023` does this and explains why in its own `Finding`.
- **Rule 2 - a fail-closed verdict must not describe a value it did not observe.** A `NotApplicable` or `Warning` branch reached because a property was absent must say what was not established, not name a value nothing returned (e.g. not "Safe Attachments for Teams is disabled (`EnableATPForSPOTeamsODB = $false`)" when the property was never returned at all - say instead that whether it is enabled was not established). On those branches the `Finding` must also say *why* an unconfirmed state is not graded as a pass, not just stop at "was not established" - `Get-METReport`'s HTML card collapses `Recommendation` behind "How to fix," so a reader scanning cards sees only the `Finding`. This scoping is deliberate: the "reported as unassessed rather than a pass" clause belongs on branches that resolve to `NotApplicable`/`Warning`, not on branches that still resolve to `Fail` - on a `Fail` branch nothing is being reported as unassessed, so the sentence would be false there. `MET-MDO002` (`Checks/MDO/MET-MDO002-SafeAttachments.ps1`) is the reference implementation for this shape.
...
Discovers check scripts the same way `Invoke-METAssessment` does (`Checks/<Category>/*.ps1`) and reads each one's `$METCheckInfo` header via static AST parsing rather than executing the check - it needs no Exchange Online/Graph/Teams module loaded and no live session, which is the point: it is what a user runs to decide whether to connect and what to run, not a query against a live tenant. Returns one `MET.CheckInfo` object per matching check (`CheckId`, `Category`, `Name`, `Severity`, `Description`, `RequiresModule`, `Script`, `Path`). `Invoke-METAssessment -ListChecks` delegates to this function, so the dry-run listing and the real run share one discovery path and can't drift apart on the checks both can parse - the one case they can disagree is a check file whose metadata header fails to parse: `Get-METCheck` skips it with a `Write-Warning` naming the file and the parse error, so it is absent from the `-ListChecks` object stream, while `Invoke-METAssessment` still attempts to run that file and reports it as an `Error` result. The warning tells a human the file is broken; a listing still can't list metadata it couldn't read.
...
| MET-EXO003 | SPF | SPF record present; not `+all` or `?all` (RFC 7208 treats neutral the same as no record, so `?all` is a Fail, not a Warning); `-all` preferred over `~all`; within 10-lookup limit and the count fully verified |
