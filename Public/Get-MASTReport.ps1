    ...
    process {
      ...
      if ([string]::IsNullOrWhiteSpace($effectiveTenantName)) {
        ...
        if ($Format -eq 'All' -and -not $OutputPath) {
            Write-Warning "Get-METReport: -Format All writes both JSON and HTML. Provide -OutputPath (a folder path) to avoid very long console output."
        }
...
.score-main{display:flex;flex-direction:column;align-items:flex-start;gap:6px}
.score-number{font-size:52px;font-weight:700;line-height:1;letter-spacing:-2px}
.score-band{font-size:11px;font-weight:700;padding:3px 10px;border-radius:10px;color:#fff;letter-spacing:.06em;text-transform:uppercase;align-self:flex-start}
...
<div class="toolbar">
  <div class="tabs">
    ...
    <div class="tab" data-tab="Controls">Controls <span class="tab-count" id="tc-controls">0</span></div>
  </div>
  <div class="filters">
...
function fmtFinding(s) {
  ...
  if (lines.length <= 1) {
    var issues = normalized.split(/;\s*/).filter(function(i){ return i.trim(); });
    if (issues.length <= 1) return esc(normalized.trim());
  ...
  return lines.map(function(line) {
    ...
    var policyName = sep !== -1 ? line.substring(0, sep).trim() : line.trim();
    var issueStr   = sep !== -1 ? line.substring(sep + 2).trim() : '';
...
function applyFilters() {
  ...
  document.getElementById('top5-section').style.display = isControls ? 'none' : '';
  document.getElementById('cards-container').style.display = isControls ? 'none' : '';
  document.getElementById('btn-collapse-all').style.display = isControls ? 'none' : '';
  if (isControls) {
  ...
  allCards.forEach(function(card) {
    ...
    const accepted = card.dataset.accepted === '1';
    ...
    else if (activeTab === 'Accepted') show = accepted;
...
function updateTabCounts() {
  ...
  allCards.forEach(function(card) {
    ...
    if (card.dataset.accepted === '1') counts.Accepted++;
  });
  ...
  document.getElementById('tc-accepted').textContent = counts.Accepted;
