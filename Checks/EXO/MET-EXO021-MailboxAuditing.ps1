...
elseif ($null -eq $auditDisabled) {
    New-METCheckResult -CheckId 'MET-EXO021' -Category EXO -Name 'Mailbox Audit Logging' `
        ...
        -Recommendation 'Confirm the state directly: Get-OrganizationConfig | Format-List AuditDisabled. If it reports true, run Set-OrganizationConfig -AuditDisabled $false, then confirm per-mailbox auditing is on (Get-Mailbox | Format-List UserPrincipalName, AuditEnabled).' `
        -ReferenceUrl 'https://learn.microsoft.com/en-us/purview/audit-mailboxes'
}
else {
    New-METCheckResult -CheckId 'MET-EXO021' -Category EXO -Name 'Mailbox Audit Logging' `
        ...
        -Recommendation 'Confirm per-mailbox auditing is on as well (Get-Mailbox | Format-List UserPrincipalName, AuditEnabled) - the organization-wide switch does not by itself guarantee every mailbox is being audited.' `
        -ReferenceUrl 'https://learn.microsoft.com/en-us/purview/audit-mailboxes'
}
