...
| MSAL version-pin to unblock in-process Graph (superseded by subprocess isolation below) | ❓ | Exhaustively checked whether any published `ExchangeOnlineManagement` version bundles `Microsoft.Identity.Client` in the `4.82.x` range that both `Microsoft.Graph.Authentication` 2.39.0 (needs `4.82.1.0`) and `MicrosoftTeams` 7.9.0 (needs `4.82.0.0`) tolerate. Every EXO version from 3.7.0 through 3.10.1 was checked: the bundled MSAL jumps directly from `4.74.1.0` (3.9.0-3.9.2) to `4.83.1.0` (3.10.0-3.10.1) with no `4.82.x` release in between. EXO 3.9.2's `4.74.1.0` also fails outright against Teams 7.9.0 (`FileLoadException` 0x80131040 - too old, not just "different"). Conclusion: no version triple of EXO+Graph+Teams coexists in one PowerShell process today across the currently-published module set - this is a genuine gap in what Microsoft has shipped, not a gap in this investigation. A pin is a net loss versus the current graceful-degradation design (EXO+Teams both stay current; only Graph degrades non-fatally), so none is being adopted. Superseded by the subprocess-isolation item below as the actual path to in-process Graph coverage. Externally corroborated by [microsoftgraph/msgraph-sdk-powershell#3394](https://github.com/microsoftgraph/msgraph-sdk-powershell/issues/3394) and [#3576](https://github.com/microsoftgraph/msgraph-sdk-powershell/issues/3576): both report the same `Microsoft.Identity.Client` assembly conflict between EXO and Graph independent of MET, confirm it's load-order-dependent (whichever module loads second fails), and #3576 notes it's a regression from EXO 3.6.0 (previously working). #3394's fix - connect Graph before EXO - doesn't contradict `Connect-METSession`'s EXO-first ordering; it's the same "load the module needing the higher MSAL version first" rule applied to a different version pair (Graph 2.30.0/EXO 3.9.0) than MET's current target versions, where EXO holds the higher requirement. Neither issue has a maintainer fix, timeline, or a version triple that resolves the three-way EXO+Graph+Teams conflict, so the conclusion above stands |
