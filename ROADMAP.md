...
| EXO019 SMTP Client Authentication | ✅ | High | SMTP AUTH enabled tenant-wide means every mailbox can submit over the protocol rather than only the ones that need it, against Microsoft's own explicit guidance. The check deliberately does *not* claim the endpoint is password-only: `SmtpClientAuthenticationDisabled` is protocol-level and SMTP AUTH supports OAuth as well as Basic, which is blocked separately by authentication policy - the finding and recommendation both say so. Checks the tenant-wide switch, then - only when that is already closed - enumerates per-mailbox overrides that re-open it. The enumeration has its own try/catch and degrades to a Warning with the error surfaced: a mailbox-enumeration failure can never abort the check, but it must not score as a clean Pass either, because the exposure the check exists to find went unverified |
...
| MSAL version-pin to unblock in-process Graph (superseded by subprocess isolation below) | ❓ | Exhaustively checked whether any published `ExchangeOnlineManagement` version bundles `Microsoft.Identity.Client` in the `4.82.x` range that both `Microsoft.Graph.Authentication` 2.39.0 (needs `4.82.1.0`) and `MicrosoftTeams` 7.9.0 (needs `4.82.0.0`) tolerate. Every EXO version from 3.7.0 through 3.10.1 was checked: the bundled MSAL jumps directly from `4.74.1.0` (3.9.0-3.9.2) to `4.83.1.0` (3.10.0-3.10.1) with no `4.82.x` release in between. EXO 3.9.2's `4.74.1.0` also fails outright against Teams 7.9.0 (`FileLoadException` 0x80131040 - too old, not just "different"). Conclusion: no version triple of EXO+Graph+Teams coexists in one PowerShell process today across the currently-published module set - this is a genuine gap in what Microsoft has shipped, not a gap in this investigation. A pin is a net loss versus the current graceful-degradation design (EXO+Teams both stay current; only Graph degrades non-fatally), so none is being adopted. Superseded by the subprocess-isolation item below as the actual path to in-process Graph coverage |
| Subprocess-isolation for Microsoft Graph checks | 🗓 | Proposed fix for the MSAL conflict above, now that a version pin is confirmed impossible: run Graph-dependent checks (currently only Teams014) in a separate `pwsh -NoProfile` child process that imports only `Microsoft.Graph.Authentication` at its own latest version - never in the same process as `ExchangeOnlineManagement`/`MicrosoftTeams` - and returns its check results to the parent (e.g. serialized as JSON over stdout) for merging into the normal result collection. Sidesteps the assembly-load conflict entirely instead of chasing a compatible version triple. Needs a design decision on: how `Connect-METSession`'s existing auth params (interactive/cert/managed identity) get forwarded into the child process without re-prompting or duplicating credential handling, and how a child-process failure surfaces (should degrade Teams014 to `NotApplicable`/`Warning` exactly as today, not abort the parent run) |
| Safe Attachments encrypted (password-protected) attachment blocking | 🗓 | Confirmed coverage gap, not a research item - the cmdlet surface is documented and stable. `Get-SafeAttachmentPolicy` exposes `EnableBlockingEncryptedAttachments` (the portal's **Block unscanned attachments**) and `QuarantineTagForBlockingEncryptedAttachments`. When Safe Attachments cannot detonate a password-protected attachment, this setting quarantines the message instead of delivering it; a password-protected archive with the password in the message body is standard commodity-malware tradecraft, so a tenant with `Action = Block` but this setting off still takes delivery of the exact payload Safe Attachments exists to stop. **Two distinct gaps.** (1) `MET-MDO002` never reads the setting - it evaluates only `Enable`/`Action` plus the global `EnableATPForSPOTeamsODB`, so an exposed tenant scores a clean Pass. (2) `MET-EXO009` (`MET-EXO009-QuarantinePolicyVerdictAlignment.ps1:96`) already enumerates `Get-SafeAttachmentPolicy | Where-Object { $_.Action -eq 'Block' }` but reads only `$p.QuarantineTag`, missing the second quarantine tag such a policy can carry - whose default, `DefaultFullAccessWithNotificationPolicy`, is a **full-access** policy, precisely the class of finding EXO009 exists to surface. Microsoft documents the setting as meaningful only when `Action = Block`, so any check must gate on that rather than flagging it unconditionally. **Implementation caveat:** the `Set-SafeAttachmentPolicy` cmdlet-reference syntax block does not list either parameter (they are documented only in the feature and quarantine-policy articles), so `Get-SafeAttachmentPolicy` may not return them on every module/service version - an absent property must therefore resolve to `NotApplicable` with the reason recorded, never `Pass`, per CLAUDE.md's absent-property rule, with `MET-MDO002` itself as the reference shape. Docs: <https://learn.microsoft.com/defender-office-365/safe-attachments-about#encrypted-password-protected-attachments-in-safe-attachments-policies> |
