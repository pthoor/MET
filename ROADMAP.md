...
| EXO011 Mail Flow Connector Hygiene | ✅ | High | `Get-InboundConnector`. Flags enabled connectors with `RequireTls=$false` or no `SenderIPAddresses`/`SenderDomains` restriction. Outbound connectors shipped out of scope (lower-confidence schema, lower risk) |
...
| MDO013 Preset vs. Custom Policy Precedence Conflicts | ✅ | High | Reuses MDO008's `Resolve-METCoverageMatrix`/`Expand-METRuleRecipients` to detect custom EOP/Safe Links/Anti-Phish rules whose targeted recipients are also covered by a Standard/Strict preset — the preset always wins, so the custom rule is silently inert for the overlap. Distinct failure mode from MDO008's coverage-gap check |
...
| EXO014 Advanced Delivery Policy Scope | ✅ | Medium | `Get-ExoPhishSimOverrideRule`. Legitimate for phishing-sim platforms, but a stale or overly broad override rule is an unfiltered inbound channel that bypasses MDO entirely. Info-only (no correct value to assert) — surfaces enabled overrides by name for periodic review |
