...
| MSAL version-pin for in-process Graph/EXO/Teams compatibility | ❓ | No published `ExchangeOnlineManagement` version bundles the `4.82.x` MSAL range used by Graph 2.39.0 and Teams 7.9.0: EXO jumps from `4.74.1.0` in 3.9.x to `4.83.1.0` in 3.10.x. MET now follows the externally reported mitigation from [microsoftgraph/msgraph-sdk-powershell#3394](https://github.com/microsoftgraph/msgraph-sdk-powershell/issues/3394) and [#3576](https://github.com/microsoftgraph/msgraph-sdk-powershell/issues/3576): connect Graph first, then EXO with `-DisableWAM`. Assembly diagnostics use a snapshot taken before any connection leg so the Graph assembly loaded deliberately by MET is not mistaken for pre-existing contamination. This ordering still needs live Windows validation, particularly with Teams 7.9.0; subprocess isolation below remains the fallback if the three-module combination cannot be made reliable in one process. |
