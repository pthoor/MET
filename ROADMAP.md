...
Design conversation about quarantine checks not distinguishing Microsoft-managed preset (Standard/Strict) configuration from admin-managed custom configuration surfaced two confirmed false-positive bugs, not just a design gap - see `docs/gap-analysis-2026-08-quarantine-policies.md` for the full matrix, citations, and implementation plan. All 4 items implemented via parallel subagent dispatch; full suite passes 292/292 tests with zero lint errors, no regressions.
...
A hardening proposal for `Connect-METSession` (`docs/deviceauth-change.md`) surfaced a confirmed cross-customer data leak path plus five supporting fixes. Every technical claim was independently verified against the actual code before acceptance (not taken on trust), and the device-code phishing risk claim was independently researched against current Microsoft guidance - see `docs/gap-analysis-2026-08-connect-session-security.md` for the full verification table, citations, and implementation plan. Implemented and verified: full suite passes 315/315 unit + 16/16 integration tests with zero lint errors, no regressions (also caught and fixed a pre-existing stale integration test asserting a 38-check count instead of the current 44).
