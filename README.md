...
For app-only auth, `-TenantId` **must be the customer's `.onmicrosoft.com` domain, not the tenant GUID** - `Connect-ExchangeOnline`'s app-only `-Organization` rejects GUIDs, and `Connect-METSession` fails fast if you pass one.
