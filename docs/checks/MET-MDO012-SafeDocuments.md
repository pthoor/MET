...
Safe Documents requires Microsoft 365 A5/E5 or Microsoft 365 E5 Security licensing; on tenants without that licensing, `EnableSafeDocs` cannot be turned on and this check reports it as a gap regardless. On some ExchangeOnlineManagement versions - and on tenants lacking that licensing - `Get-AtpPolicyForO365` may not return `EnableSafeDocs` or `AllowSafeDocsOpen` at all, or may return either property present but explicitly `$null`. Neither case says anything about the tenant's configuration: it must never be read as "off" (which would fabricate a Fail for a tenant that may not have this licensing) or, worse, as "on" for the click-through restriction (which would fabricate a Pass telling the reader that malicious-file click-through is blocked when nothing confirmed that). Both properties absent, or present but `$null`, are reported as `NotApplicable`, not asserted either way.
...
| NotApplicable | `Get-AtpPolicyForO365` returned no object, or either `EnableSafeDocs` or `AllowSafeDocsOpen` was not returned or was returned present but `$null` - the corresponding control was not established for this tenant, most often because of ExchangeOnlineManagement module version or missing Microsoft 365 A5/E5 Security licensing |
